...
export class LLMService implements ILLMService {
  ...
  private async fetchGeminiModelsInfo(
    ...
    return [{ id: "gemini-2.0-flash", name: "Gemini 2.0 Flash" }];
  }
...
export function createLLMService(modelManager: ModelManager): ILLMService {
  ...
  if (process.env.LITELLM_ENABLED === "true" && process.env.LITELLM_BASE_URL) {
    ...
    } = require("../litellm-routing/litellm-service");
    return createLiteLLMService(modelManager);
  }

  // 默认返回直接LLM服务
  console.log("[LLM Service Factory] Using direct LLM service.");
